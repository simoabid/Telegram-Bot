...
const handler = async (m, { bot}) => {
    ...
    bot.detectiveGame = bot.detectiveGame || {};
    bot.detectiveGame[m.chat] = {
...
handler.before = async (m, { bot}) => {
    if (bot.detectiveGame && bot.detectiveGame[m.chat]) {
        const respuesta = parseInt(m.text.trim());
        const { respuestaCorrecta, resultado} = bot.detectiveGame[m.chat];

        if (respuesta>= 1 && respuesta <= resultado.length) {
            delete bot.detectiveGame[m.chat];
