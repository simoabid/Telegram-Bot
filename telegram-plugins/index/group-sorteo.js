...
let handler = async (m, { bot, text, usedPrefix, command }) => {
    if (command === 'reclamar') {
        ...
        setTimeout(() => endGiveaway(groupId, bot, 'timeout'), duration);
    }
    ...
    if (command === 'enter') {
        ...
        bot.reply(m.chat, `Llegaste al sorteo!`, m);

        if (giveaway.participants.length >= giveaway.maxParticipants) {
            endGiveaway(groupId, bot, 'participants');
        }
    }
};

let endGiveaway = (groupId, bot, reason) => {
    let giveaway = giveawayData[groupId];
