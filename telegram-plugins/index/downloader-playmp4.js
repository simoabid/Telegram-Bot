...
const handler = async (m, { bot, command, args, text, usedPrefix }) => {
  if (!text) return m.reply(`*🤔Que está buscando? 🤔*\n*Ingrese el nombre de la canción*\n\n*Ejemplo:*\n${usedPrefix + command} emilia 420`);
  
  // Agregado: Reacciones para indicar que la solicitud está en progreso
  await m.react('⌛🌀');
  ...
  if (userRequests[m.sender]) return await bot.reply(m.chat, `⏳ Hey @${m.sender.split('@')[0]} espera pendejo, ya estás descargando algo 🙄\nEspera a que termine tu solicitud actual antes de hacer otra...`, userCaptions.get(m.sender) || m);
...
`;
    ...
    const PlayText = await bot.sendMessage(m.chat, { text: newDesign,  
      contextInfo:{  
    ...
    if (command === 'play' || command === 'musica') {
      ...
      if (mediaData) {
        ...
        if (fileSize > LimitAud) {
          await bot.sendMessage(m.chat, { document: isDirect ? mediaData : { url: mediaData }, mimetype: 'audio/mpeg', fileName: `${yt_play[0].title}.mp3`, contextInfo: {} }, { quoted: m });
        } else {
          await bot.sendMessage(m.chat, { audio: isDirect ? mediaData : { url: mediaData }, mimetype: 'audio/mpeg', contextInfo: {} }, { quoted: m });
        }
      } else {
        await m.react('❌');
      }
    ...
    if (command === 'play2' || command === 'video') {
      ...
      if (mediaData) {
        ...
        if (fileSize > LimitVid) {
          await bot.sendMessage(m.chat, { document: isDirect ? mediaData : { url: mediaData }, ...messageOptions }, { quoted: m });
        } else {
          await bot.sendMessage(m.chat, { video: isDirect ? mediaData : { url: mediaData }, thumbnail: yt_play[0].thumbnail, ...messageOptions }, { quoted: m });
        }
      } else {
        await m.react('❌');
      }
    ...
    if (command === 'play4' || command === 'playdoc2') {
      const { mediaData, isDirect } = await download(videoApis);
      if (mediaData) {
        await bot.sendMessage(m.chat, { document: isDirect ? mediaData : { url: mediaData }, fileName: `${yt_play[0].title}.mp4`, caption: `🔰Título: ${yt_play[0].title}`, thumbnail: yt_play[0].thumbnail, mimetype: 'video/mp4'}, { quoted: m })
      } else {
        await m.react('❌');
      }
    }
