const handler = async (m, {bot}) => {
  ...
  const hsl = `
*✧ Resultados de la caza ${bot.getName(m.sender)} ✧*
 ...
 *🐊 ${ar6} ${anti6}*                    *🐓 ${ar12} ${anti12}*`.trim();
  ...
  if (new Date - global.db.data.users[m.sender].lastberburu < 2700000) return bot.reply(m.chat, `Por favor descansá un momento para seguir cazando\n\n⫹⫺ Tiempo ${clockString(time - new Date())}\n${wm}`, m);

  setTimeout(() => {
    bot.reply(m.chat, hsl, m);
  }, 20000);

  setTimeout(() => {
    bot.reply(m.chat, `@${m.sender.split('')[0]} *${['OBJETIVO FIJADO 🎯', 'Carnada en Marcha 🍫 🍇 🍖', 'ANIMALES DETECTADOS!! 🐂 🐅 🐘 🐼', 'ANIMALES DETECTADOS!! 🐖 🐃 🐮 🐒'].getRandom()}*`, null, {mentions: [m.sender]});
  }, 18000);

  setTimeout(() => {
    bot.reply(m.chat, `@${m.sender.split('')[0]} *${['Armas lista para la Caza!!', 'Probando Armas 🔫 💣 🪓 🏹', 'CARROS PARA LA CAZA!! 🚗 🏍️ 🚜', 'TIEMPO BUENO PARA LA CAZA 🧤'].getRandom()}*`, null, {mentions: [m.sender]});
  }, 15000);

  setTimeout(() => {
    bot.reply(m.chat, `@${m.sender.split('')[0]} *${['Buscando implementos de caza...', 'Alistando todo para la caza!!', 'Estableciendo Lugar de la Caza...', 'PREPARANDO LUGAR DE CAZA!!'].getRandom()}*`, m, m.mentionedJid ? {mentions: [m.sender]} : {});
  }, 0);
