const handler = async (m, { bot}) => {
    ...
    bot.gladiadorGame = bot.gladiadorGame || {};
    bot.gladiadorGame[m.chat] = {};

    await bot.sendMessage(m.chat, { text: mensaje});
};

handler.before = async (m, { bot}) => {
    if (bot.gladiadorGame && bot.gladiadorGame[m.chat]) {
        ...
        if (eleccion>= 1 && eleccion <= estilosCombate.length) {
            ...
            bot.gladiadorGame[m.chat] = { nombre: usuario, estilo: estiloSeleccionado};

            await bot.reply(m.chat, `✅ *${usuario} ha elegido:* ${estiloSeleccionado}\n⌛ Preparándose para la batalla...`, m);
            ...
            setTimeout(() => {
                ...
                delete bot.gladiadorGame[m.chat];
}, 5000);
} else {
            await bot.reply(m.chat, "❌ *Opción inválida. Elige un número entre 1 y 13.*", m);
}
}
};
