...
const handler = async (m, { bot, isAdmin, isOwner, args, usedPrefix, command}) => {
  if (!(isAdmin || isOwner)) {
...
🕒 Puedes usar cualquier combinación: días, horas, minutos, segundos.
  ...
  setTimeout(async () => {
    ...
    await bot.groupSettingUpdate(m.chat, nuevoEstado);
    bot.reply(m.chat, nuevoEstado === 'announcement'
