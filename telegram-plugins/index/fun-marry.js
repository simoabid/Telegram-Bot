        ...
        if (isPropose) {
            ...
            if (!proposee) {
                if (userIsMarried(proposer)) {
                    return await bot.reply(m.chat, `《✧》 Ya estás casado con *${bot.getName(marriages[proposer])}*\n> Puedes divorciarte con el comando: *#divorce*`, m);
                } else {
                    throw new Error('Debes mencionar a alguien para aceptar o proponer matrimonio.\n> Ejemplo » *#marry @⁨barbpza.⁩*');
                }
            }
            if (userIsMarried(proposer)) throw new Error(`Ya estás casado con ${bot.getName(marriages[proposer])}.`);
            if (userIsMarried(proposee)) throw new Error(`${bot.getName(proposee)} ya está casado con ${bot.getName(marriages[proposee])}.`);
            if (proposer === proposee) throw new Error('¡No puedes proponerte matrimonio a ti mismo!');

            proposals[proposer] = proposee;
            const proposerName = bot.getName(proposer);
            const proposeeName = bot.getName(proposee);
            const confirmationMessage = `♡ ${proposerName} te ha propuesto matrimonio. ${proposeeName}  ¿aceptas? •(=^●ω●^=)•\n\n*Debes Responder con:*\n> ✐"Si" » para aceptar\n> ✐"No" » para rechazar.`;
            await bot.reply(m.chat, confirmationMessage, m, { mentions: [proposee, proposer] });

            confirmation[proposee] = {
                proposer,
                timeout: setTimeout(() => {
                    bot.sendMessage(m.chat, { text: '*《✧》Se acabó el tiempo, no se obtuvo respuesta. La propuesta de matrimonio fue cancelada.*' }, { quoted: m });
            ...
            await bot.reply(m.chat, `✐ ${bot.getName(m.sender)} y ${bot.getName(partner)} se han divorciado.`, m);
        }
    } catch (error) {
        await bot.reply(m.chat, `《✧》 ${error.message}`, m);
    }
}
...
handler.before = async (m) => {
    ...
    if (/^No$/i.test(m.text)) {
        ...
        return bot.sendMessage(m.chat, { text: '*《✧》Han rechazado tu propuesta de matrimonio.*' }, { quoted: m });
    }
    ...
    if (/^Si$/i.test(m.text)) {
        ...
        bot.sendMessage(m.chat, { text: `✩.･:｡≻───── ⋆♡⋆ ─────.•:｡✩
¡Se han Casado! ฅ^•ﻌ•^ฅ*:･ﾟ✧\n\n*•.¸♡ Esposo ${bot.getName(proposer)}\n*•.¸♡ Esposa ${bot.getName(m.sender)}\n\n\`Disfruten de su luna de miel\`
