...
const imageCarouselHandler = async (m, { bot, command, usedPrefix, text = "girls" }) => {
  ...
  try {
    ...
    await bot.relayMessage(m.chat, carouselMsg.message, { messageId: carouselMsg.key.id });
  } catch (error) {
    console.error(error);
    bot.reply(m.chat, "❌ Error al obtener las imágenes para el carrusel", m);
  }
};
