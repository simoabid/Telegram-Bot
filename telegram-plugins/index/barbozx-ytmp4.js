...
let handler = async (m, { bot, text, usedPrefix, command }) => {
  ...
  await bot.sendMessage(m.chat, '📀 Iniciando descarga...');
  try {
    ...
    // Send download started message instead of reaction
    await bot.sendMessage(m.chat, '✅ Descarga iniciada...');
    ...
    // Send completion message instead of reaction
    await bot.sendMessage(m.chat, '🟢 Descarga completada exitosamente');
...
// Renamed to avoid conflict with downloader-ytmp4.js
handler.help = ['bytmp4 <URL>'];
handler.command = ['bytmp4'];
