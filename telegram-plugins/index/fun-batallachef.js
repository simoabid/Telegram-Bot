...
const handler = async (m, { bot}) => {
    ...
    bot.chefBattleGame = bot.chefBattleGame || {};
    bot.chefBattleGame[m.chat] = {};

    await bot.sendMessage(m.chat, { text: mensaje});
};

handler.before = async (m, { bot}) => {
    if (bot.chefBattleGame && bot.chefBattleGame[m.chat]) {
        ...
        if (eleccion>= 1 && eleccion <= ingredientes.length) {
            ...
            bot.chefBattleGame[m.chat] = { nombre: usuario, ingrediente: ingredienteSeleccionado};

            await bot.reply(m.chat, `✅ *${usuario} ha elegido:* ${ingredienteSeleccionado}\n⌛ Preparándose para la batalla culinaria...`, m);
            ...
            setTimeout(() => {
                ...
                delete bot.chefBattleGame[m.chat];
}, 5000);
} else {
