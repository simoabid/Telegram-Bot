...
async function sendAlbumMessage(bot, jid, medias, options) {
  options = { ...options };
  ...
  await bot.relayMessage(album.key.remoteJid, album.message, { messageId: album.key.id });
  ...
  for (const i in medias) {
    ...
    await bot.relayMessage(img.key.remoteJid, img.message, { messageId: img.key.id });
...
let handler = async (m, { bot, args }) => {
  ...
  try {
    ...
    await sendAlbumMessage(bot, m.chat, medias, { caption: albumCaption, quoted: m });
