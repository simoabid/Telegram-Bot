...
if (enviando) return;
    enviando = true;
try {
   await bot.sendMessage(m.chat, global.wait);
 ...
 if (res?.result.type == 'video') {
     ...
     await bot.sendMessage(m.chat, {video: {url: res.result.media[i].result[0].url}, caption: caption}, {quoted: m});
     };
     enviando = false;
     return;
     ...
     const caption = res?.result.caption ? res.result.caption : '*Aquí tiene su imagen*';
     for (let i = 0; i < res.result.media.length; i++) {
     await bot.sendMessage(m.chat, {image: {url: res.result.media[i].url}, caption: caption}, {quoted: m});
     };
     enviando = false;
     return;
  }
