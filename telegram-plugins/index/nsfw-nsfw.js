...
if (!global.db.data.chats[m.chat].nsfw) return bot.sendMessage(m.chat, `[❗] 𝐋𝐨𝐬 𝐜𝐨𝐦𝐚𝐧𝐝𝐨𝐬 +𝟏𝟖 𝐞𝐬𝐭𝐚́𝐧 𝐝𝐞𝐬𝐚𝐜𝐭𝐢𝐯𝐚𝐝𝐨𝐬 𝐞𝐧 𝐞𝐬𝐭𝐞 𝐠𝐫𝐮𝐩𝐨.\n> 𝐬𝐢 𝐞𝐬 𝐚𝐝𝐦𝐢𝐧 𝐲 𝐝𝐞𝐬𝐞𝐚 𝐚𝐜𝐭𝐢𝐯𝐚𝐫𝐥𝐨𝐬 𝐮𝐬𝐞 .enable nsfw`, { reply_to_message_id: m.id })
...
if (!json.url) throw new Error('No se encontró contenido')
await bot.sendPhoto(m.chat, json.url, {
    caption: `*» Resultado* : ${command}`,
    reply_to_message_id: m.id
})
await bot.sendMessage(m.chat, '✅ Contenido enviado')
} catch (error) {
await bot.sendMessage(m.chat, '✖️ Error al obtener contenido', { reply_to_message_id: m.id })
}}
