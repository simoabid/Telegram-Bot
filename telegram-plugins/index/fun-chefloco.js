...
const handler = async (m, { bot}) => {
    ...
    bot.crazyChefGame = bot.crazyChefGame || {};
    bot.crazyChefGame[m.chat] = {};

    await bot.sendMessage(m.chat, { text: mensaje});
};

handler.before = async (m, { bot}) => {
    if (bot.crazyChefGame && bot.crazyChefGame[m.chat]) {
        ...
        if (eleccion>= 1 && eleccion <= caos.length) {
            ...
            bot.crazyChefGame[m.chat] = { nombre: usuario, evento: eventoSeleccionado};

            await bot.reply(m.chat, `✅ *${usuario} ha elegido:* ${eventoSeleccionado}\n⌛ La locura culinaria comienza en 3...2...1!`, m);
            ...
            setTimeout(() => {
                ...
                delete bot.crazyChefGame[m.chat];
}, 5000);
} else {
