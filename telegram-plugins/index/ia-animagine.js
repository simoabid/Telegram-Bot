...
• Si no se especifica, el ratio por defecto es "1:1"`;
        ...
        return bot.sendMessage(m.chat, caption, { reply_to_message_id: m.id });
    }
    ...
    if (!validRatios.includes(aspect_ratio)) {
        return bot.sendMessage(m.chat, `📐 *Senpai*, ¡ratio inválido!
Usa uno de estos: ${validRatios.join(', ')}.
Ejemplo: *${usedPrefix + command}* 1girl, cute, school|1:1`, { reply_to_message_id: m.id });
    }

    try {
        await bot.sendMessage(m.chat, '⏳ Generando imagen con Animagine AI...', { reply_to_message_id: m.id });
...
🖼️ *URL:* ${imageUrl}`,
        ...
        await bot.sendMessage(m.chat, '✅ Imagen generada exitosamente');

    } catch (e) {
        console.error('Error:', e.message);
        await bot.sendMessage(m.chat, `⚠️ *¡Uy, ha ocurrido un error, Senpai!* Esta función está experimentando problemas, inténtalo de nuevo más tarde 😅`, { reply_to_message_id: m.id });
    }
};
