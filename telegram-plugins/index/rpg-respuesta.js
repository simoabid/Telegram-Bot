let handler = async (m, { bot, text, usedPrefix, command }) => {
    ...
    console.log("bot.menfess", bot.menfess); 

    if (!bot.menfess || !bot.menfess[id]) {
        throw m.reply(`*⚠️ Error:* No se encontró ningún mensaje con el ID *${id}*.`);
    }

    let { dari, penerima } = bot.menfess[id];
    ...
    try {
        ...
        let sentMessage = await bot.sendMessage(dari, {
            text: teks,
            contextInfo: {
                mentionedJid: [dari],
                externalAdReply: {
        ...
        if (sentMessage) {
           return bot.reply(m.chat, '*⚡ Respuesta enviada con éxito.*\n*IDENTIFICADOR:*' + ` *${id}*`, m, fake);


            bot.menfess[id].status = true;
        } else {
