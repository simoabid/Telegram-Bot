...
let handler = async (m, { bot, command, usedPrefix, args }) => {
try {
...
const thumbnail = (await bot.getFile(image))?.data
...
await bot.reply(m.chat, captvid, m, infoReply)
infoReply.contextInfo.externalAdReply.body = "Audio descargado con éxito" // Para confirmar la descarga Jjjj

await bot.sendMessage(m.chat, { audio: { url: downloadUrl }, caption: captvid, mimetype: "audio/mpeg", contextInfo: infoReply.contextInfo, }, { quoted: m }
)} catch (error) {
