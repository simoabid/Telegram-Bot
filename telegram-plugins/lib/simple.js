...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    const _rawSendMessage = conn.sendMessage.bind(conn)
    async function toEnglish(str) {
        try {
            if (!str || typeof str !== 'string') return str
            if (global?.lang === 'en' || global?.language === 'en') {
            ...
            async value(jid, content, options = {}) {
                try {
                    ...
                    if (typeof content === 'object' && content) {
                        c = { ...content }
                        if (typeof c.text === 'string') c.text = await toEnglish(c.text)
                        if (typeof c.caption === 'string') c.caption = await toEnglish(c.caption)
                        if (typeof c.footer === 'string') c.footer = await toEnglish(c.footer)
                        if (typeof c.title === 'string') c.title = await toEnglish(c.title)
                        if (typeof c.footerText === 'string') c.footerText = await toEnglish(c.footerText)
                        if (Array.isArray(c.buttons)) {
                            c.buttons = await Promise.all(c.buttons.map(async b => {
                                if (b?.buttonText?.displayText && typeof b.buttonText.displayText === 'string') {
                                    return { ...b, buttonText: { ...b.buttonText, displayText: await toEnglish(b.buttonText.displayText) } }
                                }
                                return b
                            }))
                        }
                        if (Array.isArray(c.sections)) {
                            c.sections = await Promise.all(c.sections.map(async sec => {
                                const s = { ...sec }
                                if (typeof s.title === 'string') s.title = await toEnglish(s.title)
                                if (Array.isArray(s.rows)) {
                                    s.rows = await Promise.all(s.rows.map(async r => {
                                        const rr = { ...r }
                                        if (typeof rr.title === 'string') rr.title = await toEnglish(rr.title)
                                        if (typeof rr.description === 'string') rr.description = await toEnglish(rr.description)
                                        return rr
                                    }))
                                }
                                return s
                        ...
                        if (c?.contextInfo?.externalAdReply) {
                            let ear = c.contextInfo.externalAdReply
                            let updated = false
                            if (typeof ear.title === 'string') { ear = { ...ear, title: await toEnglish(ear.title) }; updated = true }
                            if (typeof ear.body === 'string') { ear = { ...ear, body: await toEnglish(ear.body) }; updated = true }
                            if (updated) c = { ...c, contextInfo: { ...(c.contextInfo || {}), externalAdReply: ear } }
                        }
                    } else if (typeof content === 'string') {
                        c = await toEnglish(content)
                    }
                    return _rawSendMessage(jid, c, options)
                } catch (e) {
      ...
      async value(jid, text = '', footer = '', text2 = '', messages, quoted, options) {
        ...
        reply: {
            ...
            async value(jid, text = '', quoted, options) {
                return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { ...options, text: await toEnglish(text) }, { quoted, ...options })
            }
        },
